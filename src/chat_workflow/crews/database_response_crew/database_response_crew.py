...
class database_response_crew():
	...
	@agent
	def Database_SQL_Agent(self) -> Agent:
		return Agent(
			config=self.agents_config['Database_SQL_Agent'],
			llm = self.llm,
			verbose=True,
			tools=[				
         		connectToDB, 
	...
	def Editor_Agent(self) -> Agent:
		return Agent(
			...
			backstory= '''
			    ...
			    Do Not Allow Markfown or HTML in the response.
	...
	@task
	def NLP_to_SQL(self) -> Task:
		return Task(
			config=self.tasks_config['NLP_to_SQL'],
		)
